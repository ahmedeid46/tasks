...
<?php $component = App\View\Components\Forms\Datepicker::resolve(['custom' => 'true','fieldId' => 'custom_fields_data['.e($field->name . '_' . $field->id).']','fieldRequired' => ($field->required === 'yes') ? true : false,'fieldLabel' => $field->label,'fieldName' => 'custom_fields_data['.e($field->name . '_' . $field->id).']','fieldValue' => ($model && $model->custom_fields_data['field_'.$field->id] != '') ? \Carbon\Carbon::parse($model->custom_fields_data['field_'.$field->id])->format(companyOrGlobalSetting()->date_format) : now()->format(companyOrGlobalSetting()->date_format),'fieldPlaceholder' => $field->label] + (isset($attributes) && $attributes instanceof Illuminate\View\ComponentAttributeBag ? (array) $attributes->getIterator() : [])); ?>
...
<?php endif; ?>
<?php /**PATH C:\laragon\www\worksuite\resources\views/components/forms/custom-field.blade.php ENDPATH**/ ?>